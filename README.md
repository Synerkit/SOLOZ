...
`grist-readonly` requires administrator-provided `GRIST_BASE_URL`, `GRIST_DOC_ID`, and a Viewer-only `GRIST_API_KEY`. The five staging destinations require their corresponding protected `ACTIVEPIECES_*_STAGING_WEBHOOK_URL` values. Keep those values in private instance configuration, never in this repository. The Grist helper sends GET requests only; the Activepieces helper sends one explicitly requested POST and never retries automatically.
